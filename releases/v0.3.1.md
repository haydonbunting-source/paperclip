...
- **Gemini CLI adapter** — Full local adapter support for Google's Gemini CLI. Includes API-key detection, turn-limit handling, sandbox and approval modes, and skill injection into `~/.gemini/`. Current hardened builds require explicit opt-in for yolo-style permission bypass. ([#452](https://github.com/paperclipai/paperclip/pull/452), [#656](https://github.com/paperclipai/paperclip/pull/656), @aaaaron)
...
- **Unattended permission prompt handling** — Added unattended-agent handling for permission prompts; current hardened builds require explicit opt-in before skipping permissions. ([#388](https://github.com/paperclipai/paperclip/pull/388), @ohld)
